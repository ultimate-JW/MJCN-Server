"""② 띵똥이의 조언 문구 조립 (이슈 #164, Figma 160-1144).

수강신청 테마 상세 상단 'AI 띵똥이의 조언' 멘트를 2파트로 만든다:

  stage_message : (학년, 학기) 고정 행동 멘트 — 모든 사용자 동일 (8칸 테이블)
  user_insight  : 추천 결과 신호에서 도출한 근거 멘트 — 사용자마다 다름

규칙 기반(LLM 없음). 신호 추출은 services에서, phrasing만 이 모듈에서 담당 —
나중에 LLM phrasing으로 갈아끼우려면 build_user_insight만 교체하면 된다.
"""
