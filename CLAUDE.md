# CLAUDE.md
- 이 파일은 Claude Code가 이 프로젝트를 작업할 때 참고하는 설명서 중에서도 course앱에 최적화 된 지침입니다.
- 이 파일는 main에 merge 안 할 예정으로 작업자 '이설하'의 원활한 작업을 위해 feature/9에만 저장합니다.
...
- 앱 5개:
  - **accounts**: 회원가입, 로그인, 프로필 관리 + 사용자 수강이력(`CourseHistory`/`CurrentCourse`)
  - **courses**: 학교 과목 원본 데이터(과목코드, 과목명, 학점, 학과 등), 선수과목 관계,
  시간표, 졸업요건, 추천 기능
  - **notices**: 학교 공지사항 + AI 파이프라인(분류 → 요약 → 카드 구조화, spec 9.1)
  - **information**: 외부 정보(공모전/대외활동/지원사업/교육·강의/부트캠프, spec 4.5·5.5)
  - **common**: 여러 앱이 공통으로 쓰는 코드(페이지네이션 등)
...
- 학생 정보 추가됨: 이름, 학년, 학기, 입학년도, 졸업예정 년/월, 전공, 관심사
