...
- **정렬 기준**:
  - 1순위: **학사공지(`source='academic'`) 우선 상위 노출** (#153) — 매칭 점수 무관 모든 학생이 봐야 할 행정 필수 공지
...
- **notices**: 학사공지(`source='academic'`)를 최신순으로 먼저 채운 뒤(#153) 나머지 슬롯을 관심사 매칭(5.10) 점수 내림차순 → 동점 시 최신순으로 채운 상위 3개. **학사공지가 있으면 무조건 슬롯 안에 들어감**(누락 방지) + **맞춤형(매칭) 공지를 우선 노출하되, 매칭 결과가 부족하면 최신 공지로 채운다** (매칭 0개여도 3개 반환). 출처 제한 없음(오픈톡 포함).
