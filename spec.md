...
| 항목 | 결정 |
|------|------|
| 모델 | `gpt-4o-mini` (환경변수 `OPENAI_MODEL`로 교체 가능) |
| API 키 | `.env`의 `OPENAI_API_KEY` (settings.py에서 `os.getenv` 로드) |
| 실행 | 별도 management command `manage.py process_notices_ai` |
| 트리거 | 매일 03:30 KST cron (크롤링 03:00 직후) |
| 처리 대상 | `NoticeAIResult.status='pending'` + `content_hash` 변경 감지된 항목 |
...
| 응답 파싱 | Stage 3은 OpenAI JSON mode 사용 (구조화 출력) |
