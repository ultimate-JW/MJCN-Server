...
- **학사공지 전체 노출 정책 (#153)**: `Notice.source == 'academic'`인 공지는 수강신청·등록·졸업·휴학·복학 등 모든 학생이 봐야 하는 행정 필수 공지로 간주. **매칭·태그 무관 모든 활성 사용자에게** PUSH 발송 (대시보드 / 공지 API에서도 누락 없이 노출).
      ...
      "semesters": [
        {
          ...
          "major_required":     [{"course_code": "...", "name": "...", "credits": 3, "core_area": null, "offerings": [{"id": 1, "section_no": "01", "professor": "...", "schedules": [...]}]}],
          "major_elective":     [],
          "liberal_common":     [],
          "liberal_core":       [],
          "liberal_foundation": [],
...
- **콘텐츠 매칭 대상** (`match_score` 산출용): `Notice.tags` (AI 자동 태깅 키워드)
...
```
score = 콘텐츠 태그 토큰과 하나라도 겹치는 사용자 관심사의 수
        (관심사 1개당 최대 1점)
```
...
- **카테고리 동의어 확장**: InterestArea의 12개 고정 카테고리는 매칭 시 **도메인 동의어 집합으로 확장**한다. 예: 관심사 `IT/개발`은 `{AI, 인공지능, 머신러닝, 소프트웨어, 개발, 데이터, …}`로 확장돼, LLM이 `AI`로만 태깅한 콘텐츠와도 매칭된다. 굵은 카테고리 라벨과 세밀한 도메인 키워드 사이 어휘 격차를 메운다. 동의어 목록은 코드(`common.matching.CATEGORY_SYNONYMS`)에서 관리하며 12개 카테고리 전부 대상. **카테고리 라벨에만** 적용(전공명·custom_text 자유 텍스트는 정확 토큰 유지). 큐레이션 없는 완전 의미 매칭(임베딩)은 v2 검토.
...
**학사공지 전체 fanout 예외 (#153)**:
- `Notice.source == 'academic'` 인 공지는 위 매칭 규칙을 우회하고 **활성+토글 ON 모든 사용자에게** 알림 생성
- `tags=[]` 여도 fanout 진행 (다른 카테고리는 tags=[] 시 skip)
- 학사공지는 수강신청·등록·졸업·휴학·복학 등 모든 학생이 봐야 하는 행정 필수 공지라서 누락 방지가 우선
- 백필 가드(최근 7일)는 그대로 적용 — 과거 학사공지 대량 backfill 시 알림 폭주 방지
- 알림 메시지: `'새 학사 공지가 등록되었습니다.'` (관심사 매칭 공지의 메시지와 구분)
