...
| category | CharField | 자동 분류 자동 분류(수강·졸업/학사공지/장학·등록금/공모전/취업·진로/일반질문/기타) |
...
| extracted_content | TextField(blank) | 본문이 이미지로만 구성된 경우 VLM으로 추출한 텍스트 (spec 9.1.6) |
...
- 첫 메시지 기반으로 AI가 채팅방 제목 자동 생성 및 카테고리 자동 분류
  - 카테고리 : 수강·졸업, 학사공지, 장학·등록금, 공모전, 취업·진로, 일반질문, 기타
...
- **입력**: 사용자 수강이력 + 전공 + 학년/학기 + 졸업 희망 연도 + 관심사
...
- 카테고리 자동 분류: 수강·졸업, 학사공지, 장학·등록금, 공모전, 취업·진로, 일반질문, 기타
...
[DB] Notice / Contest 레코드 (tags / categories는 JSONField로 보관)
   ↓ DRF Serializer
...
- `image_urls`는 본문 영역(`div.artclView`)에 포함된 이미지 URL을 절대경로로 정규화해 수집.
  본문이 이미지만으로 구성된 공지(`content`가 비거나 매우 짧음)에 대해 VLM 전처리(spec 9.1.6)에 사용한다.
- `tags`는 크롤링 단계에서는 빈 리스트 또는 사이트 카테고리만 채우고, AI 자동 태깅은 후속 파이프라인(spec 9.1)에서 채운다
- 저장은 `(source, url)` (Notice) / `(url,)` (Contest) 기준 upsert
- `extracted_content`는 크롤러가 채우지 않음. VLM 전처리 단계(spec 9.1.6)에서만 채워진다.
...
[전처리] content가 비거나 매우 짧고 image_urls가 있으면
         → VLM(gpt-4o-mini Vision)으로 이미지 → 텍스트 추출 (spec 9.1.6)
...
| 실행 (텍스트 파이프라인) | `manage.py process_notices_ai` — 매일 03:30 KST cron |
| 실행 (VLM 전처리) | `manage.py process_notice_images` — 03:15 KST cron (텍스트 파이프라인 직전) |
...
크롤러: artclView 영역의 <img> URL을 image_urls 배열에 저장
   ↓
process_notice_images 명령 (별도 cron 03:15 KST)
...
Notice.extracted_content에 추출 텍스트 저장
   ↓
다음 텍스트 파이프라인(03:30 KST) 실행 시 자동 재처리됨
...
- 실행: 매일 03:00 KST `manage.py crawl_notices` / `crawl_information` (운영 cron)
- 멱등성: `(source, url)` (Notice) / `(url,)` (Contest) 기준 upsert로 중복 방지
...
- [ ] **공지사항 AI 처리 파이프라인** (spec 9.1.1)
  ...
  - [ ] 매일 03:30 KST cron 등록
  - [ ] 단위 테스트: OpenAI 호출 mock, 부분 실패 복구, 재처리 트리거
- [ ] **VLM 전처리 (이미지 전용 공지 대응)** (spec 9.1.6)
  ...
  - [ ] 매일 03:15 KST cron 등록 (텍스트 파이프라인 03:30 직전)
