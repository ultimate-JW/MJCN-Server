...
- **학사공지 전체 노출 정책 (#153)**: `Notice.category == 'academic'`인 공지는 수강신청·등록·졸업·휴학·복학 등 모든 학생이 봐야 하는 행정 필수 공지로 간주. **매칭·태그 무관 모든 활성 사용자에게** PUSH 발송 (대시보드 / 공지 API에서도 누락 없이 노출).
...
- **정렬 기준**:
  - 1순위: **학사공지(`category='academic'`) 우선 상위 노출** (#153) — 매칭 점수 무관 모든 학생이 봐야 할 행정 필수 공지
  - 2순위: 관련도 점수 내림차순 (높은 것 위)
  - 3순위: `published_at` 내림차순 (최근 것 위)
...
- `Notice.category == 'academic'` 인 공지는 위 매칭 규칙을 우회하고 **활성+토글 ON 모든 사용자에게** 알림 생성
...
- **notices**: 학사공지(`category='academic'`)를 최신순으로 먼저 채운 뒤(#153) 나머지 슬롯을 관심사 매칭(5.10) 점수 내림차순 → 동점 시 최신순으로 채운 상위 3개. **학사공지가 있으면 무조건 슬롯 안에 들어감**(누락 방지) + **맞춤형(매칭) 공지를 우선 노출하되, 매칭 결과가 부족하면 최신 공지로 채운다** (매칭 0개여도 3개 반환). 출처 제한 없음(오픈톡 포함).
