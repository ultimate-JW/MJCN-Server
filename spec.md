...
| `contests` | 정보 크롤링/저장, 통합 조회 API | 4.2 |
...
- 이메일 + 비밀번호 + 비밀번호 확인 입력
- 3가지 입력 완료 후, 입력한 이메일로 인증 코드 발송 (유효기간 3분)
- 이메일 + 비밀번호 + 비밀번호 확인 입력
- 3가지 입력 완료 후, 입력한 이메일로 인증 코드 발송 (유효기간 3분)
...
**비밀번호 확인**
- 비밀번호와 불일치 시 "비밀번호가 일치하지 않습니다." 표시
...
- 카카오 로그인 전용 계정은 비밀번호 재설정 불가 → "카카오 로그인으로 가입된 계정입니다" 안내
...
- **맞춤형 정보 추천**: 새 정보 등록 시 + 마감일 3일 전 + 마감 전날
...
- 공지 북마크: 북마크한 공지사항 목록 조회
- 정보 북마크: 북마크한 정보(공모전, 지원 사업, 취업 정보 등) 목록 조회

#### 5.7.3 프로필 수정

- 기본 정보 변경: 이름, 학년, 학기, 졸업 희망 연도
...
- **contests**: 관심사 기반 정보 N개 (D-day 포함)
...
| GET | `/api/v1/chat/rooms/?category=<cat>` | O | 채팅방 폴더별 조회 (`<cat>`: 수강·졸업, 학사공지, 장학·등록금, 정보, 취업·진로, 일반질문, 기타) |
...
| Method | URL | 인증 | 설명 |
|--------|-----|------|------|
| GET | `/api/v1/contests/` | O | 정보 목록 (맞춤형 기본) |
| GET | `/api/v1/contests/?view=all` | O | 정보 전체보기 |
| GET | `/api/v1/contests/<id>/` | O | 정보 상세 |
...
- [ ] contests 앱: 정보 모델 + Serializer + 크롤러
